...
const createAsso = async (req, res) => {
    ...
    if (existingAsso) {
        ...
        let closedState = asso.closed === 'true' ? true : false
        asso.image = req.file ? req.file.path : undefined
...
const updateAsso = async (req, res) => {
    ...
    if (existingAsso) {
        ...
        let closedState = asso.closed === 'true' ? true : false
        asso.image = asso.image === 'REMOVE_IMAGE' ? null : (req.file ? req.file.path : undefined);

        for (let key in asso) {
            if (asso[key] === 'undefined') {
                asso[key] = null;
            }
        }
